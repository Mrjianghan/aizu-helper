<template>
  <div class="tags-container" v-if="homeshowtag">     
...
import { mapState, mapGetters } from "vuex";
    import storeLists from '@/api/storeTags'
...
export default {
  ...
  computed: {
    ...mapGetters(["tagWel", "tagList", "isCollapse", "tag","homeshowtag"]),
  ...
  methods: {
        ...
        closeTag(item) {
            let arr = storeLists.store[item.value];
            if(arr){
                arr.forEach(ele=>{
                    console.log( storejs.get(ele) );
                    storejs.remove(ele);
                })
            }
